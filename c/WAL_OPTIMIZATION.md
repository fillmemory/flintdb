...
### 트랜잭션 의미(고아 허용, 트리/인덱스 무결성 우선)

- FlintDB의 현재 WAL/Storage 트랜잭션은 **UPDATE/DELETE의 원자성**을 우선합니다.
  - 트랜잭션 중 UPDATE/DELETE는 origin에 즉시 반영하지 않고 dirty page로 누적 후 commit에서만 적용합니다.
  - rollback 시 dirty page는 폐기되므로 **트리 구조(루트로부터의 reachability)는 유지**되고, 변경분은 반영되지 않습니다.
- 반면 INSERT(새 페이지/새 레코드 할당)는 write-through(즉시 origin 기록)라서 rollback 시 **고아 페이지/고아 레코드가 남을 수 있습니다.**
  - 단, B+Tree/인덱스는 루트 포인터로부터 탐색하므로, 고아가 남아도 **참조되지 않으면 트리는 깨지지 않습니다.**

### recovery 부재 시 “트리 안 깨짐”을 위한 커밋 순서

- recovery 로직이 없으면, commit 도중 크래시 시점에 따라 “부분 적용”이 생길 수 있습니다.
- 이를 완화하기 위해 commit에서 **루트 포인터 레코드(통상 offset=0)를 마지막에 적용**하도록 구현했습니다.
  - 크래시가 root 적용 이전이면: 기존 루트가 유지되어 트리는 이전 상태로 정상, 새 페이지들은 고아로 남음
  - 크래시가 root 적용 이후이면: 그 이전에 다른 페이지들이 이미 적용된 상태라 새 루트 기준 트리가 일관됨
