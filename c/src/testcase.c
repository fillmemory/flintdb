...
int main(int argc, char **argv) {
    struct storage_opts opts = {
        .file = "./temp/strorage.bin", .mode = FLINTDB_RDWR, .block_bytes = 512 - 16,
...
int main(int argc, char **argv) {
    ...
    printf("ops: %lld total, %lldms, %.0f ops/sec (reads=%lld overwrites=%lld "
           "delete+insert=%lld)\n",
           (long long)M_ops, (long long)ms_ops, (double)M_ops / ((double)ms_ops / 1000.0), (long long)reads, (long long)overwrites, (long long)deletes);
...
static inline int dio_mt_is_failed(dio_mt_ctx *ctx) { return __atomic_load_n(&ctx->failed, __ATOMIC_SEQ_CST) != 0; }

static inline void dio_mt_set_failed(dio_mt_ctx *ctx) { __atomic_store_n(&ctx->failed, 1, __ATOMIC_SEQ_CST); }
...
int main(int argc, char **argv) {
    ...
    printf("mt ops: %lld total, %lldms, %.0f ops/sec (threads=%d lock_storage=%d "
           ...
           (long long)total_ops, (long long)ms_ops, (double)total_ops / ((double)ms_ops / 1000.0), threads, lock_storage, (long long)reads, (long long)overwrites, (long long)deletes);
...
int main(int argc, char **argv) {
    ...
    // 5) CREATE TABLE + meta
    const char *sql5 = "CREATE TABLE customers ("
                       " id INT64 NOT NULL,"
...
int main(int argc, char **argv) {
    ...
    struct flintdb_cursor_i64 *c = tbl->find(tbl,
...
int main(int argc, char **argv) {
    ...
    for (struct flintdb_row *r; (r = c1->next(c1, &e)) != NULL;) {
        ...
        if (count1 < 3) {
            // Print first 3 rows
            printf("Row %d: id=%lld, name=%s, price=%.2f, active=%d\n", count1 + 1, r->i64_get(r, 0, &e), r->string_get(r, 1, &e), r->f64_get(r, 2, &e), r->i8_get(r, 3, &e));
            if (e) {
                r->free(r);
                THROW_S(e);
            }
...
static void tc_finalize_group(struct tc_group_state *st) {
    ...
    printf("category=%s, total_price=%.0f, item_count=%lld, average_price=%.0f, "
           ...
           st->cur_cat, rr->f64_get(rr, 1, e), (long long)rr->i64_get(rr, 2, e), rr->f64_get(rr, 3, e), (long long)rr->i64_get(rr, 4, e), (long long)rr->i64_get(rr, 5, e), (long long)rr->i64_get(rr, 6, e));
...
int main(int argc, char **argv) {
    ...
    struct {
        ...
        {"Fruit", "Apple", 100}, {"Fruit", "Banana", 80}, {"Fruit", "Orange", 90}, {"Vegetable", "Carrot", 50}, {"Vegetable", "Broccoli", 70},
    };
...
int main(int argc, char **argv) {
    ...
    struct storage s = {0};
    struct storage_opts opts = {
        .file = "./temp/strorage.bin", .mode = FLINTDB_RDONLY, .block_bytes = 512 - 16,
        // .extra_header_bytes = 0,
        // .compact = -1,
        // .increment = -1, // 1024*1024*10,
        // .type = TYPE_DEFAULT,
    ...
    for (int i = 1; i <= 100; i++) {
        ...
        while ((rowid = cursor->next(cursor, &e)) >= 0) {
            ...
            if (i == 100) { // Only trace the last iteration
                TRACE("thread %d: read rowid=%lld => customer_id=%lld, customer_name=%s", tinfo->thread_num, rowid, customer_id, customer_name);
            }
        }
        if (cursor)
            cursor->close(cursor);
...
int main(int argc, char **argv) {
    ...
    for (i64 i = 0; i < 100; i++) {
        ...
        if (vlen != strlen(expected) || strncmp(actual, expected, vlen) != 0) {
            fprintf(stderr, "Value mismatch for key %lld: expected '%s', got '%.*s'\n", i, expected, (int)vlen, actual);
            val->free(val);
            return 1;
        }
        val->free(val);
...
void benchmark_table(int n, int val_size) {
    ...
    flintdb_meta_columns_add(m, "id", VARIANT_INT64, 8, 0, SPEC_NOT_NULL, NULL, NULL, &e);
    flintdb_meta_columns_add(m, "val", VARIANT_BYTES, val_size, 0, SPEC_NULLABLE, NULL, NULL, &e);
