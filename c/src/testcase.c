...
int main(int argc, char **argv) {
  char *e =
      NULL; // e will be pointed at TL_ERROR message buffer, no need to free
...
int main(int argc, char **argv) {
  char *e = NULL;
  struct list *list = arraylist_string_split(
      "apple&nbsp;banana&nbsp;cherry&nbsp;date&nbsp;elderberry", "&nbsp;", &e);
  if (e)
    THROW_S(e);
...
static int build_payload(char *out, size_t out_cap, i64 slot, u32 ver,
                         u32 len) {
  // Deterministic payload: header + repeated pattern.
  // Ensure there's always at least some prefix text.
  int n = snprintf(out, out_cap, "slot=%lld ver=%u ", (long long)slot,
                   (unsigned)ver);
  if (n < 0)
    return -1;
  size_t pos = (size_t)n;
...
int main(int argc, char **argv) {
  ...
  for (i64 op = 0; op < M_ops; op++) {
    ...
    if (r < 70) {
      ...
      if (n < 0 || (u32)n != lens[idx]) {
        fprintf(stderr,
                "len mismatch op=%lld idx=%lld off=%lld got=%d expected=%u\n",
                (long long)op, (long long)idx, (long long)offs[idx], n,
                (unsigned)lens[idx]);
        abort();
      }
      ...
      vers[idx]++;
      int actual =
          build_payload(payload, (size_t)max_payload + 64, idx, vers[idx], len);
      if (actual < 0) {
        fprintf(stderr, "payload build failed\n");
        abort();
      }
  ...
  printf("ops: %lld total, %lldms, %.0f ops/sec (reads=%lld overwrites=%lld "
         "delete+insert=%lld)\n",
         (long long)M_ops, (long long)ms_ops,
         (double)M_ops / ((double)ms_ops / 1000.0), (long long)reads,
         (long long)overwrites, (long long)deletes);
...
// Usage: ./bin/testcase [threads=4] [N_init=50000] [M_ops=200000] [seed=42]
// [lock_storage=1]
// - lock_storage=1: serialize storage ops (should be stable)
// - lock_storage=0: allow concurrent storage ops (expected to be unsafe; can
// expose races)
...
static void *dio_mt_run(void *arg) {
  ...
  for (i64 op = 0; op < ctx->ops; op++) {
    ...
    if (r < 70) {
      ...
      char *e = NULL;
      if (ctx->lock_storage)
        pthread_mutex_lock(&ctx->storage_mtx);
      struct buffer *rb = ctx->s->read(ctx->s, off, &e);
      if (ctx->lock_storage)
        pthread_mutex_unlock(&ctx->storage_mtx);
      if (e && *e) {
      ...
      if ((u32)n != len) {
        fprintf(stderr,
                "[T%lld] len mismatch idx=%lld off=%lld got=%d expected=%u\n",
                (long long)w->tid, (long long)idx, (long long)off, n,
                (unsigned)len);
        rb->free(rb);
        dio_mt_set_failed(ctx);
        pthread_mutex_unlock(&ctx->stripes[si]);
      ...
      u32 len =
          (u32)(16 + (xorshift64(&w->rng) % (u64)(ctx->max_payload - 16)));
      int actual =
          build_payload(payload, (size_t)ctx->max_payload + 64, idx, ver, len);
      if (actual < 0) {
...
int main(int argc, char **argv) {
  ...
  if (lock_storage == 0) {
    fprintf(stderr, "NOTE: lock_storage=0 is intentionally unsafe; failures "
                    "are expected (storage is not thread-safe).\n");
  }
  ...
  ctx.stripe_n = 1024;
  ctx.stripes =
      (pthread_mutex_t *)CALLOC((size_t)ctx.stripe_n, sizeof(pthread_mutex_t));
  assert(ctx.stripes);
  ...
  for (i64 i = 0; i < N_init; i++) {
    ...
    ctx.vers[i] = 1;
    int actual = build_payload(payload, (size_t)ctx.max_payload + 64, i,
                               ctx.vers[i], len);
    if (actual < 0)
      abort();
    buffer_wrap(payload, (u32)actual, &bb);
  ...
  dio_mt_worker *workers =
      (dio_mt_worker *)CALLOC((size_t)threads, sizeof(dio_mt_worker));
  assert(tids && workers);
  ...
  for (int i = 0; i < threads; i++) {
    ...
    workers[i].tid = i;
    workers[i].rng =
        (((u64)seed + (u64)i * 1315423911ull) ^ 0xD1B54A32D192ED03ull);
  ...
  printf("mt ops: %lld total, %lldms, %.0f ops/sec (threads=%d lock_storage=%d "
         "reads=%lld overwrites=%lld delete+insert=%lld)\n",
         (long long)total_ops, (long long)ms_ops,
         (double)total_ops / ((double)ms_ops / 1000.0), threads, lock_storage,
  ...
  s.close(&s);
  for (int i = 0; i < ctx.stripe_n; i++)
    pthread_mutex_destroy(&ctx.stripes[i]);
  ...
  if (dio_mt_is_failed(&ctx)) {
    fprintf(stderr,
            "TESTCASE_STORAGE_DIO_RANDOM_MT: FAILED (lock_storage=%d)\n",
            lock_storage);
    return 2;
  }
  return 0;
...
void btree_make(i64 max) {
  ...
  int ok = bplustree_init(&tree, path, 0, FLINTDB_RDWR, TYPE_DEFAULT, NULL,
                          i64_cmpr, NULL, &e);
  if (ok != 0) {
...
void btree_trace() {
  ...
  struct bplustree tree;
  memset(&tree, 0, sizeof(tree));
  int ok = bplustree_init(&tree, path, 0, FLINTDB_RDONLY, TYPE_DEFAULT, NULL,
                          i64_cmpr, NULL, &e);
  if (ok != 0) {
    fprintf(stderr, "init error: %s\n", e);
    return;
...
int main(int argc, char **argv) {
  ...
  flintdb_meta_columns_add(&mt, "customer_id", VARIANT_INT64, 0, 0,
                           SPEC_NULLABLE, "0", "int64 primary key", &e);
  flintdb_meta_columns_add(&mt, "customer_name", VARIANT_STRING, 255, 0,
                           SPEC_NULLABLE, "0", "", &e);

  char keys_arr[1][MAX_COLUMN_NAME_LIMIT] = {"customer_id"};
  flintdb_meta_indexes_add(&mt, PRIMARY_NAME, NULL,
                           (const char(*)[MAX_COLUMN_NAME_LIMIT])keys_arr, 1,
                           &e);
  if (e)
    THROW_S(e);
  ...
  if (e)
    THROW_S(e);
  if (!tx)
    THROW(&e, "transaction_begin failed");
...
남은 키 전체 조회해서 정합성 검증
내부 노드에서의 키 최소값 업데이트 경로가 다양한 케이스에서 잘 작동하는지
스트레스 테스트 권장.
*/
...
int main(int argc, char **argv) {
  ...
  rc = flintdb_decimal_from_string("10.00", 2, &a);
  assert(rc == 0);
  rc = flintdb_decimal_from_string("4", 0, &b);
  assert(rc == 0);
  ...
  rc = flintdb_decimal_from_string("1.00", 2, &a);
  assert(rc == 0);
  rc = flintdb_decimal_from_string("3", 0, &b);
  assert(rc == 0);
  rc = flintdb_decimal_divide(&a, &b, 4, &r);
  assert(rc == 0);
  flintdb_decimal_to_string(&r, buf, sizeof buf);
  ...
  rc = flintdb_decimal_from_string("-12.34", 2, &a);
  assert(rc == 0);
  rc = flintdb_decimal_from_string("2", 0, &b);
  assert(rc == 0);
  rc = flintdb_decimal_divide(&a, &b, 2, &r);
  assert(rc == 0);
  flintdb_decimal_to_string(&r, buf, sizeof buf);
  ...
  rc = flintdb_decimal_from_string("123.45", 2, &a);
  assert(rc == 0);
  rc = flintdb_decimal_from_string("0.6", 1, &b);
  assert(rc == 0);
  rc = flintdb_decimal_divide(&a, &b, 3, &r);
  assert(rc == 0);
  flintdb_decimal_to_string(&r, buf, sizeof buf);
  ...
  rc = flintdb_decimal_from_string("123.45", 2, &a);
  assert(rc == 0);
  rc = flintdb_decimal_from_string("0.006", 3, &b);
  assert(rc == 0);
  rc = flintdb_decimal_divide(&a, &b, 2, &r);
  assert(rc == 0);
  flintdb_decimal_to_string(&r, buf, sizeof buf);
  ...
  // 6) 0 / 7, S=3 => 0.000
  rc = flintdb_decimal_from_string("0", 0, &a);
  assert(rc == 0);
  rc = flintdb_decimal_from_string("7", 0, &b);
  assert(rc == 0);
  rc = flintdb_decimal_divide(&a, &b, 3, &r);
  assert(rc == 0);
  flintdb_decimal_to_string(&r, buf, sizeof buf);
  ...
  rc = flintdb_decimal_from_string("1", 0, &a);
  assert(rc == 0);
  rc = flintdb_decimal_from_string("0", 0, &b);
  assert(rc == 0);
  rc = flintdb_decimal_divide(&a, &b, 2, &r);
  ...
  rc = flintdb_decimal_from_string("100.00", 2, &a);
  assert(rc == 0);
  rc = flintdb_decimal_divide_by_int(&a, 4, &r);
  assert(rc == 0);
  flintdb_decimal_to_string(&r, buf, sizeof buf);
  ...
  rc = flintdb_decimal_from_string("-5.50", 2, &a);
  assert(rc == 0);
  rc = flintdb_decimal_divide_by_int(&a, 2, &r);
  assert(rc == 0);
  flintdb_decimal_to_string(&r, buf, sizeof buf);
...
int main(int argc, char **argv) {
  ...
  assert(got.data[0] == 0x12 && got.data[1] == 0x34 && got.data[2] == 0x56 &&
         got.data[3] == 0x78);
  ...
  // By current compare rule: non-equal types fallback to type id order ->
  // INT32(2) < STRING(11)
  assert(cmp_ns < 0);
  // Reverse should be > 0
...
extern int variant_decimal_add(struct flintdb_variant *target,
                               const struct flintdb_variant *other);
extern int variant_flintdb_decimal_plus(struct flintdb_variant *result,
                                        const struct flintdb_variant *a,
                                        const struct flintdb_variant *b);

static void dec_to_cstr(const struct flintdb_variant *v, char *buf, size_t n) {
  if (!v || v->type != DECIMAL) {
    snprintf(buf, n, "<non-decimal>");
    return;
  }
...
int main(int argc, char **argv) {
  ...
  assert(rc == 0);
  flintdb_variant_decimal_set(&v, d.sign, d.scale, d);
  struct flintdb_variant rhs;
  flintdb_variant_init(&rhs);
  ...
  struct flintdb_variant a;
  flintdb_variant_init(&a);
  struct flintdb_variant b;
  flintdb_variant_init(&b);
  struct flintdb_variant r;
  flintdb_variant_init(&r);
  ...
  rc = variant_flintdb_decimal_plus(&r, &a, &b);
  assert(rc == 0);
...
int main(int argc, char **argv) {
  ...
  const char *sql1 = // "SELECT DISTINCT a, b, c FROM t WHERE a=1 ORDER BY b
                     // DESC LIMIT 10";
      ...
      "/* Multi-line \n"
      "comment */\n"
      "FROM 'table name' USE INDEX(IX_SECOND DESC) WHERE a=1 ORDER BY b DESC "
      "LIMIT 10";
  ...
  const char *sql2 =
      "INSERT INTO foo (a,b) VALUES ('x', NULL) WHERE a>0 LIMIT 5";
  ...
  const char *sql5 =
      ...
      " PRIMARY KEY (id),"
      " KEY ix_name (name)"
      ") STORAGE=file INCREMENT=1M CACHE=2M COMPRESSOR=zstd HEADER=ABSENT "
      "DELIMITER=, QUOTE=\" NULL=\\N FORMAT=csv DATE=YYYY-MM-DD WAL=TRUNCATE";
  ...
  // date remains as provided; meta_new sets default to current date, but
  // sql_to_meta overwrote when provided
...
int main(int argc, char **argv) {
  ...
  flintdb_variant_string_ref_set(&v2, v1.value.b.data, v1.value.b.length,
                                 v1.value.b.sflag);
...
int main(int argc, char **argv) {
  ...
  DEBUG("Column 'customer_id' at index: %d",
        flintdb_column_at(&mt, "customer_id"));
  DEBUG("Column 'CUSTOMER_ID' at index: %d",
        flintdb_column_at(&mt, "CUSTOMER_ID"));
  DEBUG("Column 'customer_name' at index: %d",
        flintdb_column_at(&mt, "customer_name"));
  DEBUG("Column 'CUSTOMER_NAME' at index: %d",
        flintdb_column_at(&mt, "CUSTOMER_NAME"));
...
int main(int argc, char **argv) {
  ...
  flintdb_meta_indexes_add(&mt, PRIMARY_NAME, NULL,
                           (const char(*)[MAX_COLUMN_NAME_LIMIT])keys_arr, kcnt,
                           &e);
  if (e)
    THROW_S(e);
...
static void build_meta(struct flintdb_meta *m, const char *name,
                       enum fileformat ff, char delim, char quote,
                       const char *nilstr) {
  char *e = NULL;
  memset(m, 0, sizeof(*m));
  ...
  flintdb_meta_columns_add(m, "id", VARIANT_INT64, 8, 0, SPEC_NULLABLE, "", "",
                           NULL);
  flintdb_meta_columns_add(m, "name", VARIANT_STRING, 16, 0, SPEC_NULLABLE, "",
                           "", NULL);
  flintdb_meta_columns_add(m, "amount", VARIANT_DOUBLE, 8, 0, SPEC_NULLABLE, "",
                           "", NULL);
  flintdb_meta_columns_add(m, "ts", VARIANT_DATE, 0, 0, SPEC_NULLABLE, "", "",
                           NULL);
  m->absent_header = 1;
  m->delimiter = delim;
  m->quote = quote;
...
static void assert_row_eq(struct flintdb_row *a, struct flintdb_row *b) {
  ...
  for (int i = 0; i < a->length; i++) {
    enum flintdb_variant_type t = (a->meta && a->meta->columns.length > i)
                                      ? a->meta->columns.a[i].type
                                      : a->array[i].type;
    if (t == VARIANT_DATE) {
...
int main(int argc, char **argv) {
  ...
  // 노말라이즈 비교: 문자열/숫자 캐스팅 동일성 위해 meta 타입 기준으로 비교
  // 수행
  assert_row_eq(r1, r2);
  r1->free(r1);
  r2->free(r2);
  ...
  // Validate on-wire layout matches Java BIN formatter (no padding for var-len,
  // proper tags)
  {
    struct buffer chk;
    ...
    chk.skip(&chk, 8);
    // col1: STRING "Alice" without padding; next should be DOUBLE tag
    // immediately
  ...
  // Also validate NIL handling for var-len: set name=NULL -> tag=0 and skip to
  // next tag
  {
    struct flintdb_row *rn = make_row(&mb);
    // Force NIL directly into the underlying variant for STRING column (bypass
    // row.set casting)
...
int main(int argc, char **argv) {
  ...
  flintdb_meta_columns_add(&m, "price", VARIANT_DECIMAL, 16, 2, SPEC_NULLABLE,
                           "", "", NULL);
  struct flintdb_row *r = flintdb_row_new(&m, &e);
  // set string with scale > target; should truncate via cast (use row.set to
  // honor meta type)
...
int main(int argc, char **argv) {
  ...
  flintdb_meta_columns_add(&m, "id", VARIANT_INT64, 8, 0, SPEC_NULLABLE, "", "",
                           NULL);
  flintdb_meta_columns_add(&m, "msg", VARIANT_STRING, 64, 0, SPEC_NULLABLE, "",
                           "", NULL);
  struct formatter f;
...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < 10000; i++) {
    ...
    struct flintdb_row *r = flintdb_row_new(&mt, &e);
    TRACE("after flintdb_row_new(%d), r=%p, e=%s", i, (void *)r,
          e ? e : "NULL");
    if (e)
      THROW_S(e);
    r->i64_set(r, 0, i + 1, &e);
...
int main(int argc, char **argv) {
  ...
  struct flintdb_table *tbl =
      flintdb_table_open(tablename, FLINTDB_RDONLY, NULL, &e);
  if (e)
    THROW_S(e);
  if (!tbl)
    THROW(&e, "table_open failed");
  ...
  struct flintdb_cursor_i64 *c =
      tbl->find(tbl,
                "USE INDEX(PRIMARY DESC) WHERE customer_id > 5000 AND "
                "customer_id < 5007 LIMIT 10",
                &e);
  if (e)
    THROW_S(e);
  if (c) {
    ...
    // Close the cursor to release any underlying resources (including bplustree
    // cursor)
    c->close(c);
    c = NULL;
  }
...
int main(int argc, char **argv) {
  ...
  // Create meta for test data: id(INT64), name(STRING), price(DOUBLE),
  // active(INT8)
  struct flintdb_meta m = flintdb_meta_new("test_parquet", &e);
  if (e)
    THROW_S(e);

  flintdb_meta_columns_add(&m, "id", VARIANT_INT64, 0, 0, SPEC_NULLABLE, "",
                           "Row ID", &e);
  if (e)
    THROW_S(e);
  flintdb_meta_columns_add(&m, "name", VARIANT_STRING, 128, 0, SPEC_NULLABLE,
                           "", "Name field", &e);
  if (e)
    THROW_S(e);
  flintdb_meta_columns_add(&m, "price", VARIANT_DOUBLE, 0, 0, SPEC_NULLABLE, "",
                           "Price value", &e);
  if (e)
    THROW_S(e);
  flintdb_meta_columns_add(&m, "active", VARIANT_INT8, 0, 0, SPEC_NULLABLE, "1",
                           "Active flag", &e);
  if (e)
    THROW_S(e);
  ...
  if (e) {
    ...
    printf("  Example: flintdb_genericfile_open(\"file.tsv\", FLINTDB_RDWR, "
           "&meta, &e)\n\n");
    flintdb_meta_close(&m);
    free(e);
    PRINT_MEMORY_LEAK_INFO();
  ...
  if (!f)
    THROW(&e, "Failed to open Parquet file");
  ...
  printf("Wrote %d rows in %lld ms (%.0f ops/sec)\n", num_rows,
         time_elapsed(&watch), time_ops(num_rows, &watch));
...
int main(int argc, char **argv) {
  ...
  if (e)
    THROW_S(e);
  if (!m)
    THROW(&e, "Failed to get file metadata");
  ...
  if (e)
    THROW_S(e);
  if (!c1)
    THROW(&e, "Failed to create cursor");
  ...
  for (struct flintdb_row *r; (r = c1->next(c1, &e)) != NULL;) {
    ...
    if (count1 < 3) {
      ...
      printf("Row %d: id=%lld, name=%s, price=%.2f, active=%d\n", count1 + 1,
             r->i64_get(r, 0, &e), r->string_get(r, 1, &e),
             r->f64_get(r, 2, &e), r->i8_get(r, 3, &e));
      if (e) {
        r->free(r);
        THROW_S(e);
      }
  ...
  printf("Scanned %d rows in %lld ms (%.0f ops/sec)\n", count4,
         time_elapsed(&watch4), time_ops(count4, &watch4));
  printf("Sum of IDs: %lld, Average price: %.2f\n", sum_ids,
         count4 > 0 ? sum_prices / count4 : 0.0);
  c4->close(c4);
  c4 = NULL;
...
static int tc_sortable_add_row(struct flintdb_filesort *sorter,
                               struct flintdb_meta *m, i64 id, const char *name,
                               int age, char **e) {
  struct flintdb_row *r = flintdb_row_new(m, e);
  if (e && *e)
    return -1;
...
static int tc_sortable_cmp_id_asc(const void *ctx, const struct flintdb_row *a,
                                  const struct flintdb_row *b) {
  (void)ctx;
  char *ce = NULL;
...
int main(int argc, char **argv) {
  ...
  if (e)
    THROW_S(e);
  flintdb_meta_columns_add(&m, "id", VARIANT_INT64, 0, 0, SPEC_NULLABLE, "0",
                           "", &e);
  if (e)
    THROW_S(e);
  flintdb_meta_columns_add(&m, "name", VARIANT_STRING, 64, 0, SPEC_NULLABLE, "",
                           "", &e);
  if (e)
    THROW_S(e);
  flintdb_meta_columns_add(&m, "age", VARIANT_INT32, 0, 0, SPEC_NULLABLE, "0",
                           "", &e);
  if (e)
    THROW_S(e);
...
static struct flintdb_meta tc_build_meta_1(enum flintdb_variant_type t,
                                           char **e) {
  struct flintdb_meta m = flintdb_meta_new("", e);
  if (e && *e)
    return m;
  flintdb_meta_columns_add(&m, "v", t, (t == VARIANT_STRING ? 32 : 0), 0, NULL,
                           NULL, e);
  return m;
}
...
int main(int argc, char **argv) {
  ...
  // COUNT
  {
    ...
    struct flintdb_aggregate_func *f =
        flintdb_func_count("v", NULL, VARIANT_NULL, cond, &e);
    if (e || !f)
      THROW_S(e);
    for (int i = 0; i < 5; i++) {
  ...
  // DISTINCT_COUNT exact
  {
    ...
    struct flintdb_aggregate_condition cond = {0};
    struct flintdb_aggregate_func *f =
        flintdb_func_distinct_count("v", NULL, VARIANT_NULL, cond, &e);
    if (e || !f)
      THROW_S(e);
  ...
  // DISTINCT_HLL_COUNT approx (check rough bounds)
  {
    ...
    struct flintdb_aggregate_condition cond = {0};
    struct flintdb_aggregate_func *f =
        flintdb_func_distinct_hll_count("v", NULL, VARIANT_NULL, cond, &e);
    if (e || !f)
      THROW_S(e);
    i64 vals[] = {1, 1, 2, 3, 3, 3, 4};
    for (size_t i = 0; i < sizeof(vals) / sizeof(vals[0]); i++) {
  ...
  // SUM, AVG
  {
    ...
    if (e || !r)
      THROW_S(e);
    struct flintdb_aggregate_condition cond = {0};
    struct flintdb_aggregate_func *fs =
        flintdb_func_sum("v", NULL, VARIANT_NULL, cond, &e);
    struct flintdb_aggregate_func *fa =
        flintdb_func_avg("v", NULL, VARIANT_NULL, cond, &e);
    if (e || !fs || !fa)
      THROW_S(e);
    i64 vals[] = {1, 2, 3};
    for (size_t i = 0; i < sizeof(vals) / sizeof(vals[0]); i++) {
  ...
  // MIN, MAX on integers
  {
    ...
    if (e || !r)
      THROW_S(e);
    struct flintdb_aggregate_condition cond = {0};
    struct flintdb_aggregate_func *fmin =
        flintdb_func_min("v", NULL, VARIANT_NULL, cond, &e);
    struct flintdb_aggregate_func *fmax =
        flintdb_func_max("v", NULL, VARIANT_NULL, cond, &e);
    if (e || !fmin || !fmax)
      THROW_S(e);
    i64 vals[] = {5, 1, 3};
    for (size_t i = 0; i < sizeof(vals) / sizeof(vals[0]); i++) {
  ...
  // FIRST, LAST with NIL in between
  {
    ...
    if (e || !r)
      THROW_S(e);
    struct flintdb_aggregate_condition cond = {0};
    struct flintdb_aggregate_func *ffirst =
        flintdb_func_first("v", NULL, VARIANT_NULL, cond, &e);
    struct flintdb_aggregate_func *flast =
        flintdb_func_last("v", NULL, VARIANT_NULL, cond, &e);
    if (e || !ffirst || !flast)
      THROW_S(e);
    // 10, NIL, 30
    r->i64_set(r, 0, 10, &e);
  ...
  // MIN/MAX with strings
  {
    ...
    if (e)
      THROW_S(e);
    assert(strcmp(flintdb_variant_string_get(fmin->result(fmin, NULL, &e)),
                  "a") == 0);
    assert(strcmp(flintdb_variant_string_get(fmax->result(fmax, NULL, &e)),
                  "c") == 0);
    fmin->free(fmin);
    fmax->free(fmax);
    r->free(r);
  ...
  // Test 9: GROUP BY with aggregate functions
  {
    ...
    if (e)
      THROW_S(e);
    flintdb_meta_columns_add(&m, "category", VARIANT_STRING, 32, 0,
                             SPEC_NULLABLE, NULL, NULL, &e);
    flintdb_meta_columns_add(&m, "amount", VARIANT_INT64, 0, 0, SPEC_NULLABLE,
                             NULL, NULL, &e);
    if (e)
      THROW_S(e);
    ...
    struct flintdb_aggregate_groupby *gb =
        groupby_new("category", "category", VARIANT_STRING, &e);
    struct flintdb_aggregate_func *fc =
        flintdb_func_count("amount", "cnt", VARIANT_NULL, cond, &e);
    struct flintdb_aggregate_func *fs =
        flintdb_func_sum("amount", "total", VARIANT_NULL, cond, &e);
    if (e || !gb || !fc || !fs)
      THROW_S(e);

    // Allocate arrays on heap (aggregate_new stores these pointers)
    struct flintdb_aggregate_groupby **gbs =
        (struct flintdb_aggregate_groupby **)CALLOC(
            1, sizeof(struct flintdb_aggregate_groupby *));
    struct flintdb_aggregate_func **funcs =
        (struct flintdb_aggregate_func **)CALLOC(
            2, sizeof(struct flintdb_aggregate_func *));
    if (!gbs || !funcs)
      THROW_S(e);
    gbs[0] = gb;
    funcs[0] = fc;
    ...
    struct flintdb_aggregate *agg =
        aggregate_new("test_groupby", gbs, 1, funcs, 2, &e);
    if (e || !agg)
      THROW_S(e);
...
static int tc_cmp_category(const struct flintdb_row *a,
                           const struct flintdb_row *b, void *ctx) {
  (void)ctx;
  char *ce = NULL;
  const char *sa =
      ((struct flintdb_row *)a)->string_get((struct flintdb_row *)a, 0, &ce);
  if (ce)
    return 0;
  const char *sb =
      ((struct flintdb_row *)b)->string_get((struct flintdb_row *)b, 0, &ce);
  if (ce)
    return 0;
  if (!sa && !sb)
    return 0;
...
static void tc_finalize_group(struct tc_group_state *st) {
  if (!st || !st->have_group)
    return;
  char **e = st->e;
  if (!st->f_sum || !st->f_cnt || !st->f_avg || !st->f_dcnt || !st->f_hll ||
      !st->f_rb) {
    TRACE("finalize_group missing funcs: sum=%p cnt=%p avg=%p dcnt=%p hll=%p "
          "rb=%p",
          (void *)st->f_sum, (void *)st->f_cnt, (void *)st->f_avg,
          (void *)st->f_dcnt, (void *)st->f_hll, (void *)st->f_rb);
    return;
  }
  ...
  rr->i64_set(
      rr, 4,
      flintdb_variant_i64_get(st->f_dcnt->result(st->f_dcnt, NULL, e), e), e);
  rr->i64_set(rr, 5,
              flintdb_variant_i64_get(st->f_hll->result(st->f_hll, NULL, e), e),
              e);
  rr->i64_set(rr, 6,
              flintdb_variant_i64_get(st->f_rb->result(st->f_rb, NULL, e), e),
              e);

  // Print
  printf("category=%s, total_price=%.0f, item_count=%lld, average_price=%.0f, "
         "distinct=%lld, distinct_hll=%lld, distinct_rb=%lld\n",
         st->cur_cat, rr->f64_get(rr, 1, e), (long long)rr->i64_get(rr, 2, e),
         rr->f64_get(rr, 3, e), (long long)rr->i64_get(rr, 4, e),
         (long long)rr->i64_get(rr, 5, e), (long long)rr->i64_get(rr, 6, e));
...
int main(int argc, char **argv) {
  ...
  if (e)
    THROW_S(e);
  flintdb_meta_columns_add(&m, "category", VARIANT_STRING, 50, 0, SPEC_NULLABLE,
                           NULL, NULL, &e);
  if (e)
    THROW_S(e);
  flintdb_meta_columns_add(&m, "item", VARIANT_STRING, 100, 0, SPEC_NULLABLE,
                           NULL, NULL, &e);
  if (e)
    THROW_S(e);
  flintdb_meta_columns_add(&m, "price", VARIANT_UINT32, 0, 0, SPEC_NULLABLE,
                           NULL, NULL, &e);
  if (e)
    THROW_S(e);

  struct flintdb_genericfile *f =
      flintdb_genericfile_open(path, FLINTDB_RDWR, &m, &e);
  if (e || !f)
  ...
  struct {
    ...
    u32 price;
      ...
      {"Fruit", "Apple", 100},       {"Fruit", "Banana", 80},
      {"Fruit", "Orange", 90},       {"Vegetable", "Carrot", 50},
  ...
  // 2) Read back using same handle and group-by category using a linked hashmap
  // (preserves insertion order)

  // 3) Group scan with hashmap and compute aggregates
  // Result meta: category STRING(50), total_price DOUBLE, item_count INT64,
  // average_price DOUBLE,
  //              item_count_distinct INT64, item_count_distinct_hll INT64,
  //              item_count_distinct_rb INT64
  struct flintdb_meta rm = flintdb_meta_new("groupby_result", &e);
  if (e)
    THROW_S(e);
  flintdb_meta_columns_add(&rm, "category", VARIANT_STRING, 50, 0,
                           SPEC_NULLABLE, NULL, NULL, &e);
  flintdb_meta_columns_add(&rm, "total_price", VARIANT_DOUBLE, 0, 0,
                           SPEC_NULLABLE, NULL, NULL, &e);
  flintdb_meta_columns_add(&rm, "item_count", VARIANT_INT64, 0, 0,
                           SPEC_NULLABLE, NULL, NULL, &e);
  flintdb_meta_columns_add(&rm, "average_price", VARIANT_DOUBLE, 0, 0,
                           SPEC_NULLABLE, NULL, NULL, &e);
  flintdb_meta_columns_add(&rm, "item_count_distinct", VARIANT_INT64, 0, 0,
                           SPEC_NULLABLE, NULL, NULL, &e);
  flintdb_meta_columns_add(&rm, "item_count_distinct_hll", VARIANT_INT64, 0, 0,
                           SPEC_NULLABLE, NULL, NULL, &e);
  flintdb_meta_columns_add(&rm, "item_count_distinct_rb", VARIANT_INT64, 0, 0,
                           SPEC_NULLABLE, NULL, NULL, &e);
  if (e)
    THROW_S(e);
  ...
  flintdb_meta_columns_add(&m_item, "v", VARIANT_STRING, 100, 0, NULL, NULL,
                           &e);
  ...
  struct hashmap *groups =
      linkedhashmap_new(16, hashmap_string_hash, hashmap_string_cmpr);
  if (!groups)
    THROW(&e, "groups hashmap alloc failed");

  // For this tutorial test, feed from in-memory rows[] instead of reading back
  for (size_t i = 0; i < sizeof(rows) / sizeof(rows[0]); i++) {
    TRACE("feed row %zu: cat=%s item=%s price=%u", i, rows[i].cat, rows[i].item,
          rows[i].price);
    ...
    struct tc_group_state *gst =
        (gv == HASHMAP_INVALID_VAL) ? NULL : (struct tc_group_state *)(var)gv;
    TRACE("after groups->get for key=%s -> %p (raw=0x%llx)", k, (void *)gst,
    ...
    if (gst == NULL) {
      ...
      gst->have_group = 1;
      gst->f_sum =
          flintdb_func_sum("v", "total_price", VARIANT_DOUBLE, cond, &e);
      gst->f_cnt =
          flintdb_func_count("v", "item_count", VARIANT_NULL, cond, &e);
      gst->f_avg =
          flintdb_func_avg("v", "average_price", VARIANT_DOUBLE, cond, &e);
      gst->f_dcnt = flintdb_func_distinct_count("v", "item_count_distinct",
                                                VARIANT_NULL, cond, &e);
      gst->f_hll = flintdb_func_distinct_hll_count(
          "v", "item_count_distinct_hll", VARIANT_NULL, cond, &e);
      gst->f_rb = flintdb_func_distinct_count("v", "item_count_distinct_rb",
                                              VARIANT_NULL, cond, &e);
      if (e) {
        FREE(k);
        THROW_S(e);
      }
      groups->put(groups, HASHMAP_CAST_KEY(k), HASHMAP_CAST_VAL((var)gst),
                  tc_group_free);
    } else {
      FREE(k);
    }
...
int main(int argc, char **argv) {
  ...
  f = flintdb_genericfile_open("../c/temp/tpch_lineitem.tsv.gz", FLINTDB_RDONLY,
                               NULL, &e);
  if (e || !f)
...
int main(int argc, char **argv) {
  ...
  m.absent_header = 1;
  strcpy(m.nil_str, "\\N");
  flintdb_meta_columns_add(&m, "id", VARIANT_INT64, 0, 0, SPEC_NULLABLE, NULL,
                           NULL, &e);
  flintdb_meta_columns_add(&m, "name", VARIANT_STRING, 32, 0, SPEC_NULLABLE,
                           NULL, NULL, &e);
  flintdb_meta_columns_add(&m, "v", VARIANT_INT64, 0, 0, SPEC_NULLABLE, NULL,
                           NULL, &e);
  if (e)
    THROW_S(e);
...
int main(int argc, char **argv) {
  ...
  if (e)
    THROW_S(e);
  flintdb_meta_columns_add(&m, "i64_col", VARIANT_INT64, 0, 0, SPEC_NULLABLE,
                           NULL, NULL, &e);
  flintdb_meta_columns_add(&m, "f64_col", VARIANT_DOUBLE, 0, 0, SPEC_NULLABLE,
                           NULL, NULL, &e);
  flintdb_meta_columns_add(&m, "decimal_col", VARIANT_DECIMAL, 8, 2,
                           SPEC_NULLABLE, NULL, NULL, &e);
  flintdb_meta_columns_add(&m, "str_col", VARIANT_STRING, 64, 0, SPEC_NULLABLE,
                           NULL, NULL, &e);
  flintdb_meta_columns_add(&m, "date_col", VARIANT_DATE, 0, 0, SPEC_NULLABLE,
                           NULL, NULL, &e);
  flintdb_meta_columns_add(&m, "time_col", VARIANT_TIME, 0, 0, SPEC_NULLABLE,
                           NULL, NULL, &e);
  if (e)
    THROW_S(e);
  ...
  printf("%lldrows, %s, %.0fops, total_bytes=%zu\n", max, tbuf,
         time_ops(max, &watch), total_bytes);
...
int main(int argc, char **argv) {
  ...
  // Prepare a set of variant pairs to exercise compare fast paths and mixed
  // paths
  enum { K = 10 };
  ...
  // 5) DECIMAL vs DECIMAL (different scale -> compare by
  // sign,scale,length,data)
...
int main(int argc, char **argv) {
  ...
  //  move_on_get: 1 to move MRU on get (default 1), 0 to keep insertion order
  //  for faster gets buckets: hashsize (default 131072*8) capacity: LRU max
  ...
  cache =
      lruhashmap_new(buckets, capacity, &hashmap_int_hash, &hashmap_int_cmpr);
...
int main(int argc, char **argv) {
  ...
  t = flintdb_table_open("../c/temp/c/tpch_lineitem" TABLE_NAME_SUFFIX,
                         FLINTDB_RDWR, &meta, &e);
  if (e || !t)
    THROW(&e, "table_open failed: %s", e ? e : "unknown error");

  f = flintdb_genericfile_open("../c/temp/tpch/lineitem.tbl.gz", FLINTDB_RDONLY,
                               NULL, &e);
  if (e || !f)
...
int main(int argc, char **argv) {
  ...
  for (i64 i = 0; i < 5 && i < nrows; i++) {
    ...
    if (r) {
      ...
      LOG("row[%lld]: l_orderkey = %u", i,
          flintdb_variant_u32_get(orderkey, &e));
    }
  }
...
static void *thread_reader_run(void *arg) {
  ...
  for (int i = 1; i <= 100; i++) {
    ...
    while ((rowid = cursor->next(cursor, &e)) >= 0) {
      ...
      if (e) {
        WARN("thread %d: read failed for rowid=%lld: %s", tinfo->thread_num,
             rowid, e);
        break;
      }
      ...
      if (i == 100) { // Only trace the last iteration
        TRACE(
            "thread %d: read rowid=%lld => customer_id=%lld, customer_name=%s",
...
static int tc_decimal_to_cstr(const struct flintdb_decimal *d, char *out,
                              size_t cap) {
  if (!out || cap == 0)
    return -1;
  out[0] = '\0';
...
int main(int argc, char **argv) {
  ...
  const char *create_table_sql = "CREATE TABLE IF NOT EXISTS tpch_lineitem (\n"
                                 ...
                                 "PRIMARY KEY (l_orderkey, l_linenumber)\n"
                                 ");";
  char *sql = "INSERT INTO tpch_lineitem (l_orderkey, l_partkey, l_suppkey, "
...
// 기본값은 MEMORY 입니다.
  ...
  for (struct flintdb_row *r; (r = cur->next(cur, &e)) != NULL;) {
    ...
    if (rc == SQLITE_OK) {
      ...
      rc = sval ? sqlite3_bind_text(stmt, 9, sval, -1, SQLITE_TRANSIENT)
                : sqlite3_bind_null(stmt, 9);
    }
    if (rc == SQLITE_OK) {
      sval = r->string_get(r, 9, NULL);
      rc = sval ? sqlite3_bind_text(stmt, 10, sval, -1, SQLITE_TRANSIENT)
    ...
    if (rc == SQLITE_OK) {
      if (r->is_nil(r, 10, NULL))
        ...
        tc_date_to_yyyy_mm_dd(r->date_get(r, 10, NULL), datestr,
                              sizeof(datestr));
    ...
    if (rc == SQLITE_OK) {
      if (r->is_nil(r, 11, NULL))
        ...
        tc_date_to_yyyy_mm_dd(r->date_get(r, 11, NULL), datestr,
                              sizeof(datestr));
    ...
    if (rc == SQLITE_OK) {
      if (r->is_nil(r, 12, NULL))
        ...
        tc_date_to_yyyy_mm_dd(r->date_get(r, 12, NULL), datestr,
                              sizeof(datestr));
    ...
    if (rc == SQLITE_OK) {
      ...
      rc = sval ? sqlite3_bind_text(stmt, 14, sval, -1, SQLITE_TRANSIENT)
                : sqlite3_bind_null(stmt, 14);
    }
    if (rc == SQLITE_OK) {
      sval = r->string_get(r, 14, NULL);
      rc = sval ? sqlite3_bind_text(stmt, 15, sval, -1, SQLITE_TRANSIENT)
                : sqlite3_bind_null(stmt, 15);
    }
    if (rc == SQLITE_OK) {
      sval = r->string_get(r, 15, NULL);
      rc = sval ? sqlite3_bind_text(stmt, 16, sval, -1, SQLITE_TRANSIENT)
...
#ifdef TESTCASE_WISCKEY
// ./testcase.sh TESTCASE_WISCKEY
#include "wisckey.h"

int main(int argc, char **argv) {
  char *e = NULL;
  struct wisckey wk;
  const char *db_path = "./temp/test_wisckey";

  // Clean up previous test files
  char buf[PATH_MAX];
  snprintf(buf, sizeof(buf), "%s.idx", db_path);
  unlink(buf);
  snprintf(buf, sizeof(buf), "%s.vlog", db_path);
  unlink(buf);

  int rc = wisckey_open(&wk, db_path, FLINTDB_RDWR, &e);
  if (rc != 0) {
    fprintf(stderr, "Failed to open WiscKey: %s\n", e ? e : "unknown error");
    return 1;
  }
  ...
  for (i64 i = 0; i < 100; i++) {
    char val_str[64];
    snprintf(val_str, sizeof(val_str), "Value for key %lld", i);
    struct buffer bb = {0};
    buffer_wrap(val_str, (u32)strlen(val_str), &bb);
    wisckey_put(&wk, i, &bb, &e);
    if (e) {
      fprintf(stderr, "Put failed for key %lld: %s\n", i, e);
      return 1;
    }
  }
  ...
  printf("Testing Get...\n");
  for (i64 i = 0; i < 100; i++) {
    struct buffer *val = wisckey_get(&wk, i, &e);
    if (!val) {
      fprintf(stderr, "Get failed for key %lld: %s\n", i, e ? e : "not found");
      return 1;
    }
    char expected[64];
    snprintf(expected, sizeof(expected), "Value for key %lld", i);
    u32 vlen = val->remaining(val);
    const char *actual = val->array_get(val, vlen, NULL);
    if (vlen != strlen(expected) || strncmp(actual, expected, vlen) != 0) {
      fprintf(stderr,
              "Value mismatch for key %lld: expected '%s', got '%.*s'\n", i,
              expected, (int)vlen, actual);
      val->free(val);
      return 1;
    }
    val->free(val);
  ...
  wisckey_delete(&wk, 50, &e);
  struct buffer *val50 = wisckey_get(&wk, 50, &e);
  if (val50) {
    fprintf(stderr, "Delete failed: key 50 still exists\n");
    val50->free(val50);
    return 1;
  }

  wisckey_close(&wk);
  printf("WiscKey tests passed!\n");

  PRINT_MEMORY_LEAK_INFO();
  return 0;
}
#endif // TESTCASE_WISCKEY

#ifdef TESTCASE_WISCKEY_PERF
...
void benchmark_wisckey(int n, int val_size) {
  ...
  // Robust cleanup
  const char *suffixes[] = {".index",         ".vlog",       ".idx",
                            ".index.flintdb", ".index.desc", ".index.i.primary",
                            ".index.i.id",    ".wal",        ".index.wal"};
  for (int i = 0; i < sizeof(suffixes) / sizeof(suffixes[0]); i++) {
    snprintf(buf, sizeof(buf), "%s%s", path, suffixes[i]);
    unlink(buf);
  }

  wisckey_open(&wk, path, FLINTDB_RDWR, &e);

  char *val_ptr = malloc(val_size);
  memset(val_ptr, 'A', val_size);
  ...
  for (int i = 0; i < 1000 && i < n; i++) {
    wisckey_put(&wk, (i64)i, &val_bb, &e);
    val_bb.position = 0;
  }

  // Write
  STOPWATCH_START(w_timer);
  for (int i = 0; i < n; i++) {
    wisckey_put(&wk, (i64)i, &val_bb, &e);
    val_bb.position = 0;
  }
  u64 w_time = time_elapsed(&w_timer);
  printf("Write: %lld ms (%.2f ops/sec)\n", w_time,
         (double)n * 1000.0 / w_time);

  // Read (Random)
  STOPWATCH_START(r_timer);
  for (int i = 0; i < n; i++) {
    i64 key = rand() % n;
    struct buffer *rb = wisckey_get(&wk, key, &e);
    if (rb)
      rb->free(rb);
  }
  u64 r_time = time_elapsed(&r_timer);
  printf("Read:  %lld ms (%.2f ops/sec)\n", r_time,
         (double)n * 1000.0 / r_time);

  wisckey_close(&wk);
  free(val_ptr);
}

void benchmark_table(int n, int val_size) {
  char *e = NULL;
  const char *path = "./temp/perf_tbl";
  char buf[PATH_MAX];

  // Robust cleanup
  const char *suffixes[] = {".flintdb", ".desc", ".i.primary", ".i.id", ".wal"};
  for (int i = 0; i < sizeof(suffixes) / sizeof(suffixes[0]); i++) {
    snprintf(buf, sizeof(buf), "%s%s", path, suffixes[i]);
    unlink(buf);
  }

  struct flintdb_meta *m = flintdb_meta_new_ptr("perf_tbl", &e);
  // Use compact mode to limit block size and prevent i16 overflow for large
  // values
  m->compact = 4096;

  flintdb_meta_columns_add(m, "id", VARIANT_INT64, 8, 0, SPEC_NOT_NULL, NULL,
                           NULL, &e);
  flintdb_meta_columns_add(m, "val", VARIANT_BYTES, val_size, 0, SPEC_NULLABLE,
                           NULL, NULL, &e);
  char pk_keys[1][MAX_COLUMN_NAME_LIMIT] = {"id"};
  flintdb_meta_indexes_add(m, PRIMARY_NAME, "bptree", pk_keys, 1, &e);
  ...
  printf("--- Standard Table Performance (N=%d, ValSize=%d) ---\n", n,
         val_size);

  // Warm-up
  for (int i = 0; i < 1000 && i < n; i++) {
    struct flintdb_row *r = flintdb_row_pool_acquire(m, &e);
    r->i64_set(r, 0, (i64)i, &e);
    r->bytes_set(r, 1, val_ptr, val_size, &e);
    t->apply(t, r, 0, &e);
    flintdb_row_pool_release(r);
  }
  ...
  for (int i = 0; i < n; i++) {
    ...
    const struct flintdb_row *r = t->one(t, PRIMARY_INDEX, 2, pk_args, &e);
  }
  u64 r_time = time_elapsed(&r_timer);
  printf("Read:  %lld ms (%.2f ops/sec)\n", r_time,
         (double)n * 1000.0 / r_time);
