...
typedef char *(*reader_get_metadata_t)(void *reader, const char *key, char **error);
...
typedef int (*schema_builder_add_metadata_t)(void *builder, const char *key, const char *value);
...
struct arrow_functions {
    ...
    reader_get_metadata_t reader_get_metadata;
    ...
    schema_builder_add_metadata_t schema_builder_add_metadata;
...
static int arrow_load_library(char **e) {
    ...
    const char *lib_names[] = {"libflintdb_parquet.dylib", // macOS
...
#define LOAD_SYMBOL(name, sym)                                                                                                                                                                                                                                                                             \
    ...
    LOAD_SYMBOL(reader_get_metadata, "flintdb_parquet_reader_get_metadata");
    ...
    LOAD_SYMBOL(schema_builder_add_metadata, "flintdb_parquet_schema_builder_add_metadata");
...
static const char *flintdb_type_to_arrow_format(enum flintdb_variant_type type) {
    ...
    case VARIANT_TIME:
        return "tts"; // time64[microseconds]
    default:
...
static i64 parquetfile_write(struct flintdb_genericfile *me, struct flintdb_row *r, char **e) {
    ...
    if (!priv->writer_opened) {
        ...
        // Serialize FlintDB meta to SQL string and add as Parquet metadata
        char schema_sql[SQL_STRING_LIMIT] = {0};
        if (flintdb_meta_to_sql_string(&priv->meta, schema_sql, sizeof(schema_sql), e) != 0) {
            g_arrow.schema_builder_free(schema_builder);
            THROW(e, "Failed to serialize meta to SQL string");
        }

        // Add FlintDB schema as metadata in the Parquet file
        if (g_arrow.schema_builder_add_metadata) {
            g_arrow.schema_builder_add_metadata(schema_builder, "flintdb.schema", schema_sql);
            g_arrow.schema_builder_add_metadata(schema_builder, "flintdb.version", "1.0");
        }
...
struct flintdb_genericfile *parquetfile_open(const char *file, enum flintdb_open_mode mode, const struct flintdb_meta *meta, char **e) {
    ...
    if (NULL == meta) {
        ...
        // Use a temporary reader to get metadata
        void *reader = g_arrow.reader_open_file(file, &error_msg);
        if (reader) {
            schema_sql = g_arrow.reader_get_metadata(reader, "flintdb.schema", &error_msg);
            g_arrow.reader_close(reader);
    ...
    meta_done:;
        ...
        // FLINTDB_RDWR mode: just use provided meta (will be embedded in parquet on first write)
        priv->meta = *meta;
