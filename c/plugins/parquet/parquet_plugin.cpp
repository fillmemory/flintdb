...
struct ParquetReader {
    ...
    int64_t num_rows;
    std::shared_ptr<arrow::KeyValueMetadata> metadata; // File-level metadata
};
...
void *flintdb_parquet_reader_open(const char *path, char **error) {
    ...
    try {
        ...
        if (kv_metadata) {
            reader->metadata = std::const_pointer_cast<arrow::KeyValueMetadata>(kv_metadata);
        }
...
char *flintdb_parquet_reader_get_metadata(void *reader, const char *key, char **error) {
    if (!reader || !key) {
        set_error(error, "Reader or key is null");
        return nullptr;
    }
    ...
    try {
        ...
        // Search for the key in metadata
        if (r->metadata->Contains(key)) {
            std::string value = r->metadata->Get(key).ValueOrDie();
        ...
        set_error(error, std::string("Exception reading metadata: ") + e.what());
        return nullptr;
    }
}
...
void *flintdb_parquet_writer_open(const char *path, struct ArrowSchema *schema_c, char **error) {
    ...
    try {
        ...
        // The schema metadata is automatically written to Parquet file metadata
        // by parquet::arrow::FileWriter when the schema contains metadata
        auto maybe_writer = parquet::arrow::FileWriter::Open(*writer->schema, arrow::default_memory_pool(), writer->output);
        if (!maybe_writer.ok()) {
...
void *flintdb_parquet_schema_builder_new(void) { return new SchemaBuilder(); }
...
struct ArrowSchema *flintdb_parquet_schema_builder_build(void *builder) {
    ...
    try {
        ...
        // Build schema with or without metadata
        std::shared_ptr<arrow::Schema> schema;
        if (!b->metadata_pairs.empty()) {
            // Create KeyValueMetadata from pairs
            std::vector<std::string> keys, values;
            for (const auto &pair : b->metadata_pairs) {
                keys.push_back(pair.first);
                values.push_back(pair.second);
            }
            auto metadata = std::make_shared<arrow::KeyValueMetadata>(keys, values);
            schema = arrow::schema(b->fields, metadata);
        } else {
            schema = arrow::schema(b->fields);
        }
...
int flintdb_parquet_schema_builder_add_metadata(void *builder, const char *key, const char *value) {
    ...
    try {
        auto b = static_cast<SchemaBuilder *>(builder);
        b->metadata_pairs.emplace_back(key, value);
        return 0;
    } catch (...) {
        return -1;
    }
