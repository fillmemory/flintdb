...
public final class HashTable implements Table {
	...
	public static Table open( //
		...
		return new HashTable(Meta.make(file, meta), meta, Meta.OPEN_RDWR, logger);
	}
	 ...
	 * @param mode file access mode (Meta.OPEN_RDWR | OPEN_RDONLY or OPEN_RDONLY)
	 * @param logger logger for operations and error reporting
	...
	private boolean open(final int mode) throws IOException { // , java.nio.channels.OverlappingFileLockException
		if (this.storage == null) {
			// System.out.println("FileTable open : " + mode + ", " + ((Meta.OPEN_RDWR & mode) > 0 ? "OPEN_RDWR" : "OPEN_RDONLY"));
	...
	static final class HashKey implements Sorter {
		...
		public HashKey(final File indexFile, //
			...
			this.hash = new HashFile(indexFile, 1024 * 1024, (Meta.OPEN_RDWR & mode) > 0, (1024 * 1024 * 5), comparator);
		}
